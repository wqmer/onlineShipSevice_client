...
const nameMapCompoment = (props, state, item) => {
  ...
  let selectIV =
    props.sender_information.panel_title != "未填写"
      ? props.sender_information[item.key]
      : undefined;
  ...
  return (
    <Form.Item
      ...
      onBlur={(e) => {
        ...
        props.onBlurToRedux(obj, "sender_information", item.key);
      }}
      help={item.help}
      rules={item.rule}
          ...
          {row_content.map((item, index) => {
            return (
              <Col key={item.key} span={item.span_value}>
                {nameMapCompoment(props, state, item)}{" "}
              </Col>
            );
          })}{" "}
        </Row>
...
class SenderAddressForm extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (_.isEqual(current_form, next_form) && !this.state.is_first_render)
      return false;
...
export default connect(mapStateToProps, mapDispatchToProps)(SenderAddressForm);

...
class MyTable extends React.Component {
  ...
  //请求表格数据，并展示页面
  fetch_data(
    api_url_pagniate = this.props.api_url["get_data_pignate"],
    state = this.state
  ) {
    ...
    post(api_url_pagniate, {
      ...
      .catch((error) => {
        notification.error(
          format.notfication_remote_server_error(handle_error(error).message)
        );
  ...
  handle_action(_id, type) {
    this.setState({ is_fetching: true });
    post(handle_request(_id)[type].url, handle_request(_id)[type].body)
      .then((payload) => {
        let data_amount = this.state.total - payload.data.n;
        let result = define_current_page(
          data_amount,
        ...
        let update_content = {
          ...
          selectedRowKeys: this.state.selectedRowKeys.filter(
            (item) =>
              !handle_request(_id)[type].body[this.props.row_key].includes(item)
          ),
        };
  ...
  check_all_data(api_url_pagniate, state = this.state) {
    ...
    post(api_url_pagniate, {
      ...
      .then((payload) => {
        let check = {
          selectedRowKeys: [
            ...payload.data.docs.map((item) => item[this.props.row_key]),
          ],
        };
        this.setState({
          ...state,
  ...
  handleCheck = (action_type) =>
    this.handle_action(this.state.selectedRowKeys, action_type);
  ...
  handle_filter(filter_content, filter_tag, type = "add") {
    ...
    if (type == "add") {
      ...
      let filter_tags = _.uniq(
        this.state.filter_tags.concat([filter_tag])
      ).filter((item) => item != "is_all");
      this.fetch_data(this.props.api_url["get_data_pignate"], {
        ...this.state,
        filter: update,
        filter_tags,
      ...
      let filter_tags = _.uniq(this.state.filter_tags).filter(
        (item) => item != filter_tag
      );
  ...
  handle_search = (string) => {
    let filter_tags = _.uniq(
      this.state.filter_tags.concat("is_searching")
    ).filter((item) => item != "is_all");
    this.fetch_data(this.props.api_url["get_data_pignate"], {
      ...this.state,
  ...
  handle_clear_search = () => {
    let filter_tags = this.state.filter_tags.filter(
      (item) => item != "is_searching"
    );
    ...
    return (
      <div style={{ display: "inline-block", marginLeft: 100 }}>
        <Button
          ...
          onClick={() => this.handleCheck(this.props.layout_button.action[0])}
          type="primary"
          icon={<UploadOutlined />}
          style={{ fontSize: "12px" }}
        >
          {this.props.layout_button.batch[0]}
        </Button>
        <Button
          size="small"
          hidden={this.state.selectedRowKeys.length == 0}
          onClick={() => this.handleCheck(this.props.layout_button.action[1])}
          type="danger"
          icon={<DeleteOutlined />}
          style={{ fontSize: "12px", marginLeft: "10px" }}
        >
          {this.props.layout_button.batch[1]}
        </Button>
      </div>
    );
  };
  ...
  handle_message = (search_string) => {
    let message = search_string ? (
      <span>
        <Text strong>搜索结果</Text>：匹配{" "}
    ...
    return (
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <span>
          {message}
          <span> 一共 {this.state.total} </span> 记录。已经勾选{" "}
          ...
          {this.state.selectedRowKeys.length == 0 ? (
            ...
            <a onClick={this.handleCleanCheck} style={{ marginLeft: "30px" }}>
              取消选择
            </a>
          )}{" "}
        </span>
      ...
      selections: [
        {
          ...
          text: "选择全部",
          onSelect: () =>
            this.check_all_data(this.props.api_url["get_data_pignate"]),
        },
      ],
    };
      ...
      <div>
        {this.props.filter_content ? (
          <div style={{ marginBottom: "16px" }}>
            <Filter
              ...
              handle_filter={(data, filter_tag, type) =>
                this.handle_filter(data, filter_tag, type)
              }
        ...
        >
          ...
          <Table
            ...
            //       onDoubleClick: event => {},
            //       onContextMenu: event => {},
            //       onMouseEnter: event => {this.setState({row_shows_icon:record._id})}, // 鼠标移入行
            //       onMouseLeave: event => {this.setState({row_shows_icon:undefined})},
            //     };
            //   }}
            title={
              !this.props.buttons
                      ...
                      <div
                        style={{
                          textAlign: this.props.style
                            ? this.props.style.button_position
                            : "right",
                        }}
                      >
                        <Space
                        ...
                        >
                          {this.props.buttons
                            ? this.props.buttons.map((item) => item.component)
                            : undefined}
                        </Space>
                      </div>
                    </div>
            ...
            expandable={{
              ...
              //   expandedRowRender: (record) => (
              //     <p style={{ paddingLeft: 100 }}>test</p>
              //   ),
              //   rowExpandable: (record) => {
              //       if (record.parcel) return record.parcel.parcelList.length > 1;
              //   },
              rowExpandable: (record) => {
                return this.props.isRowExpandable
                  ? this.props.isRowExpandable(record)
                  : false;
              },
        ...
        <ActionDrawer
          check_data={this.state.selectedRowKeys}
